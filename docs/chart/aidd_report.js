  ...
  const scenarios = {
    "depot-master": {
      ...
      contextNotes: [
        ...
        "AsIs は商品マスタ一覧+登録（合計 60.3h）を以下の係数で按分: データモデル ×5、API ×4、UX ×1.1、フロント ×2.5、レビュー・動作確認 ×1.5。複雑度プレミアム（3 タブ・2 階層・マトリクス）が +20-30% 想定。<strong>後で担当者の判断で調整予定。</strong>",
  ...
  const SCENARIO_OVERRIDES = {
    ...
    'wi-if-PR19': {
      ...
      bugCategories: [
        ...
        { key: "edge",   label: "エッジケース漏れ (未検索・0件・小数棄却・全件棄却)",  color: "#8b5cf6", asis: 3, tobe: 1 },
        { key: "spec",   label: "仕様解釈ミス (RP020行構成・水源=出荷元・取込役割)",   color: "#ec4899", asis: 3, tobe: 1 },
        { key: "ui",     label: "UI 細部の不整合 (非活性・パレット色内訳の誤ラベル)",   color: "#06b6d4", asis: 2, tobe: 1 },
      ],
      bugRateNote: "※ 幹線手配スケジュール入出力 約 1.6 KLOC(BE importLineHauls 約520行＋FE エクスポート/取込/色修正 約1000行)。当初エクスポートは画面ピボットを1:1 CSV化する設計だったが受入基準(帳票RP020)未達と判明しRP020再集計へ是正。取込(RP019)はexceljsパース→対象月upsert(全件棄却・合算)。色内訳が出現順割当で誤ラベルのバグも修正(実pallet_color解決)。暫定写像は PW-194_2/_3/_4 に記録・要業務確認。",
      tasks: {
        'api-impl':    { asis: 6,    loc: 520,   tobe: 1.0,               agents: ["バックエンドAI"] },
        'api-test':    { asis: 2,    loc: 240,   tobe: 0.3,               agents: ["バックエンドテスターAI"] },
        'api-review':  { asis: 0.4,             tobe: 0.25, tobeEng: 0.25, agents: ["バックエンドレビュワーAI", "エンジニア"] },
    ...
    'wi-screen-DM03FE009': {
      ...
      bugRateNote: "※ ○○○配送状況 約2.1 KLOC(FE 約830行＋BE 約578行＋DB(schema) 約137行)。<strong>「便」データ構造自体がチケット確認事項として未確定</strong>(高ブロッカー、PM確認待ちのまま暫定実装で継続)。<strong>共有DBのマイグレーション履歴と実スキーマの乖離</strong>(他ブランチ由来とみられる破壊的DDL混在)を検知し、安全のためDB反映を保留(questions/PW-183.tsv・db-architectメモに記録)。",
      tasks: {
