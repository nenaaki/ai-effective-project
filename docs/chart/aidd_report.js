  ...
  const SCENARIO_OVERRIDES = {
    ...
    'wi-if-PR15': {
      ...
      bugCategories: [
        ...
        { key: "edge",   label: "エッジケース漏れ (不正行・重複・新規キー不可)",    color: "#8b5cf6", asis: 3, tobe: 1 },
        { key: "spec",   label: "仕様解釈ミス (稼働区分の基準日・前方一致)",        color: "#ec4899", asis: 3, tobe: 1 },
        { key: "ui",     label: "UI 細部の不整合 (表示件数セレクタ・ちらつき)",      color: "#06b6d4", asis: 3, tobe: 2 },
      ],
      bugRateNote: "※ エリア-郵便番号対応表（IF）約 1.9 KLOC(BE 約700行＋FE 約1200行)。レビュー/テストで実不具合を検出→修正: ①<strong>稼働区分が基準日(effectiveDate)を無視し常に当日</strong>→Service層で基準日導出 ②<strong>表示件数セレクタが効かない死んだUI</strong>→useState化 ③<strong>ローディング中に一覧がちらつく</strong>→previousData保持。全件棄却(不正1行で全件未登録)・更新のみ(新規キーはエラー)は実機検証で確認。未確定は questions/PW-191.tsv に記録し縮退なし。",
      tasks: {
        'data-design': { asis: 0.5,             tobe: 0.1,  tobeEng: 0,    agents: ["DBアーキテクトAI"] },
        'data-impl':   { asis: 0.5,  loc: 5,     tobe: 0.1,  tobeEng: 0.1,  agents: ["DBアーキテクトAI", "エンジニア"] },
        'api-impl':    { asis: 8,    loc: 750,   tobe: 0.4,  tobeEng: 0,    agents: ["バックエンドAI"] },
        'api-test':    { asis: 1.5,  loc: 0,     tobe: 0,                   agents: ["バックエンドAI"] },
        'ux':          { asis: 2.0,             tobe: 0.1,  tobeEng: 0,    agents: ["プランナーAI"] },
        'fe-design':   { asis: 1.0,             tobe: 0,                   agents: ["フロントエンドAI"] },
        'fe-impl':     { asis: 24,   loc: 1200,  tobe: 0.6,  tobeEng: 0,    agents: ["フロントエンドAI"] },
    ...
    'wi-if-PR19': {
      lede: "AIDD 試行「エクスポート用幹線手配スケジュール（IF / PR19 / PW-194）」の開発工数比較。<strong>幹線スケジュール画面のエクスポートボタンで、表示中データを帳票RP020レイアウトのCSVとしてダウンロード</strong>（既存 lineHauls／ピボット変換／CSV基盤を流用・DB/BE変更なし）。SFTP転送は対象外。<strong>git worktree による3件並行開発</strong>。<strong>ToBe は実測</strong>(time-log): 計 <strong>1.7h</strong>（実装系統A 0.7h＋動作確認 1.0h）。AsIs は実績コード量(概算)から推定。",
      bugCategories: [
        { key: "syntax", label: "構文/型エラー",                            color: "#ef4444", asis: 1, tobe: 0 },
        { key: "logic",  label: "ロジックバグ (ピボット直列化・列順・項目計)",   color: "#f59e0b", asis: 2, tobe: 0 },
        { key: "edge",   label: "エッジケース漏れ (未検索・0件・エスケープ)",     color: "#8b5cf6", asis: 2, tobe: 0 },
        { key: "spec",   label: "仕様解釈ミス (RP020行構成・水源=出荷元)",       color: "#ec4899", asis: 2, tobe: 1 },
        { key: "ui",     label: "UI 細部の不整合 (エクスポートボタン非活性)",     color: "#06b6d4", asis: 1, tobe: 1 },
      ],
      bugRateNote: "※ エクスポート用幹線手配（IF）約 0.14 KLOC(FE 約137行・既存資産流用のため小規模)。画面表示のピボット(scheduleCards)をそのままCSV直列化する設計で表示とCSVの乖離が構造的に起きない。RP020の3群行(積込/取卸/パレット回収)はデータに区別が無く再構成不可のため<strong>画面表示22行を1:1でCSV化</strong>(縮退せず・PW-194_2で記録)。",
      tasks: {
        'api-impl':    { asis: 0,               tobe: 0,                   agents: ["—"] },
        'ux':          { asis: 0.5,             tobe: 0.05, tobeEng: 0,    agents: ["プランナーAI"] },
        'fe-design':   { asis: 0.3,             tobe: 0,                   agents: ["フロントエンドAI"] },
        'fe-impl':     { asis: 2.7,  loc: 137,   tobe: 0.5,  tobeEng: 0,    agents: ["フロントエンドAI"] },
        'fe-review':   { asis: 0.25,            tobe: 0.15, tobeEng: 0,    agents: ["フロントエンドレビュワーAI"] },
        'verify':      { asis: 1.0,             tobe: 1.0,  tobeEng: 1.0,  agents: ["フロントエンドテスターAI", "エンジニア"] },
      },
      contextNotes: [
        "対象: エクスポート用幹線手配スケジュール（IF / PR19 / PW-194 / 配送管理・DL）。幹線スケジュール画面(FE025・稼働中)のエクスポートボタンを本実装。既存 lineHauls クエリ・表示ピボット(buildScheduleCards)・CSV基盤(utils/csv)を流用し、BE/DB変更なし。SFTP転送は別IFとして対象外。",
        "<strong>ToBe は実測</strong> (time-log.jsonl・2026-07-09): 実装 系統A <strong>0.7h</strong>(frontendのみ・reviewer/tester)＋動作確認 <strong>1.0h</strong> ＝ 計 <strong>1.7h</strong>。3件worktree並列のため按分値。",
        "<strong>手戻り</strong>: frontend-reviewerがMinor1(disabled時ツールチップ文言・任意)のみ。frontend-testerが実サーバで13観点pass(CSV BOM/列順/水源=出荷元名/22行ピボット/RFC4180を実DL検証)。Critical/Major 0。",
        "<strong>AsIs は実績コード量(概算)から推定</strong>: フロント 約137行→約2.7h(既存資産流用のため小規模)＋UX/設計。レビュー・動作確認は人間工数として計上。合計 約4.8h → ToBe 1.7h(<strong>削減 約65%</strong>)。既存資産流用で元々小規模のため削減率は他より低い。LOCは概算。",
    ...
    'wi-screen-DM13FE047': {
      lede: "AIDD 試行「再配送依頼」(FE047 / PW-190) の開発工数比較。<strong>お客様WEBの照会結果画面</strong>を状態で出し分け（訪問不在=再配送フォーム／完了済み・保管期限切れ=案内）、<strong>未ログイン公開</strong>で再配送日時予約・置き配設定を行う。配送予定変更を<strong>共通Service化しドライバーアプリと統合</strong>（既知の時間帯保存不具合 PW-163_8 も解消）・スキーマ変更なし。<strong>git worktree による3件並行開発</strong>。<strong>ToBe は実測</strong>(time-log): 計 <strong>2.6h</strong>（実装系統A 1.6h＋動作確認 1.0h）。AsIs は実績コード量(概算)から推定。",
      bugCategories: [
        { key: "syntax", label: "構文/型エラー",                                        color: "#ef4444", asis: 3, tobe: 0 },
        { key: "logic",  label: "ロジックバグ (二重予約・配送日1日ズレ・予約後状態遷移)",     color: "#f59e0b", asis: 5, tobe: 3 },
        { key: "edge",   label: "エッジケース漏れ (日付レンジ無制限DoS・不正UUIDで500)",      color: "#8b5cf6", asis: 3, tobe: 2 },
        { key: "spec",   label: "仕様解釈ミス (Prismaエラー変換・置き配NG顧客・締切)",        color: "#ec4899", asis: 3, tobe: 1 },
        { key: "ui",     label: "UI 細部の不整合 (カレンダー○△×・2タブ・完了モーダル)",      color: "#06b6d4", asis: 3, tobe: 0 },
      ],
      bugRateNote: "※ 再配送依頼 約 2.0 KLOC(BE 約800行＋FE 約1000行)。レビュー/テストの各ゲートで実不具合6件を検出→修正: ①楽観ロックが非原子で<strong>二重予約が通る</strong>→updateManyのwhere句にversionを含めアトミック化 ②<strong>配送予定日が選択日より1日ズレて保存</strong>(JST0時instant→@db.DateがUTC前日)→JST暦日→UTC深夜0時に正規化＋応答はDB値 ③予約後<strong>005000(再配達予定)で再照会がnull</strong>→statusType分岐追加 ④公開APIの<strong>日付レンジ無制限(DoS)</strong>→上限93日 ⑤<strong>非UUIDのtimeSlotIdで500</strong>→@IsUUID＋事前チェックのBadRequest変換 ⑥Service層のPrismaエラー未catch。取得元/業務未確定は questions/PW-190.tsv(_1〜_10)に記録し縮退なし。",
      tasks: {
        'data-design': { asis: 0,               tobe: 0,                    agents: ["—"] },
        'data-impl':   { asis: 0,    loc: 0,     tobe: 0,                    agents: ["—"] },
        'api-impl':    { asis: 8,    loc: 800,   tobe: 0.4,  tobeEng: 0,     agents: ["バックエンドAI"] },
        'api-test':    { asis: 1.5,  loc: 0,     tobe: 0,                    agents: ["バックエンドAI"] },
        'api-review':  { asis: 1,               tobe: 0.3,  tobeEng: 0.1,   agents: ["バックエンドレビュワーAI", "バックエンドテスターAI"] },
        'ux':          { asis: 3,               tobe: 0.1,  tobeEng: 0.1,   agents: ["プランナーAI", "エンジニア"] },
        'fe-design':   { asis: 0.5,             tobe: 0,                    agents: ["プランナーAI"] },
        'fe-impl':     { asis: 20,   loc: 1000,  tobe: 0.5,  tobeEng: 0,     agents: ["フロントエンドAI"] },
        'fe-test':     {                                                     agents: ["—"] },
        'fe-review':   { asis: 0.3,             tobe: 0.2,  tobeEng: 0.1,   agents: ["フロントエンドレビュワーAI", "エンジニア"] },
        'verify':      { asis: 2,               tobe: 1.0,  tobeEng: 1.0,   agents: ["フロントエンドテスターAI", "バックエンドテスターAI", "エンジニア"] },
      },
      contextNotes: [
        "対象画面: 再配送依頼 (FE047 / PW-190 / DM13 お客様WEB)。訪問不在=2タブの再配送フォーム(通常/指定場所)＋カレンダー○△×＋確認モーダル、完了済み・保管期限切れ・該当なしは PW-182 実装を統合。未ログイン公開(お問い合わせ番号を認可の根拠)。公開予約Mutation＋置き配設定＋空き状況算出、配送予定変更の共通Service化(driver updateDeliverySchedule と統合・PW-163_8スタブ解消)。スキーマ変更なし。",
        "<strong>ToBe は実測</strong> (time-log.jsonl・2026-07-09): 実装 系統A <strong>1.6h</strong>(planner→backend→frontend、reviewer/tester差し戻し修正を含む)＋動作確認 <strong>1.0h</strong> ＝ 計 <strong>2.6h</strong>。3件worktree並列のため按分値。",
        "<strong>手戻り（レビュー/テストで検出・修正）</strong>: backend-reviewerがMajor3(DoS/楽観ロック非原子/Prismaエラー未catch)、backend-testerが非UUID500・予約後005000のnull、frontend-testerが<strong>配送予定日1日ズレ(JST/UTC)</strong>を検出→いずれも修正・再検証で解消。driver予定変更(共通化先)も同時に是正。",
        "<strong>AsIs は実績コード量(概算)から推定</strong>: バックエンド 約800行→約9.5h(実装8＋テスト1.5)、フロント 約1000行→20h。レビュー・動作確認は人間工数として計上。合計 約36h → ToBe 2.6h(<strong>削減 約93%</strong>)。LOCは概算。",
      ],
    },
  };
