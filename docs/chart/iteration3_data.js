...
const ITERATION3_DATA = {
  ...
  tasks: [
    ...
    { name: "通知UI(ベル・一覧・既読)", base: "notify", person: 2, group: "適用", code: "N4", owner: "FE", deps: ["通知サービス・API"], asis: 26.7, plan: 4.9, tobe: 0, status: "着手中", progress: 0.8, desc: "ヘッダのベル（未読件数）・通知一覧・既読操作・該当画面への遷移。admin共通レイアウトへ組み込む。／9/9時点で実装8割（申告）。動作確認と修正が残るため未完了。工数は夜間ループの塊から切り出せていないため計上なし。" },
    { name: "業務イベント連携(帳票完了/取込エラー)", base: "notify", person: 2, group: "適用", code: "N5", owner: "BE", deps: ["配信チャネル抽象(アプリ内/メール/SMS)"], asis: 19.1, plan: 3.5, tobe: 0, status: "着手中", progress: 0.8, desc: "既存基盤の完了・失敗イベントを通知へ接続（帳票/CSV生成完了、ファイル取込エラー、SMS配信失敗）。③の失敗検知の受け口にもなる。／9/9時点で実装8割（申告）。動作確認と修正が残るため未完了。工数は夜間ループの塊から切り出せていないため計上なし。" },
    ...
    { name: "AI改善工数②", base: "reserve", person: 2, group: "予備工数", code: "AI2", owner: "両", deps: [], asis: 20.0, plan: 20.0, tobe: 1.53, status: "着手中", progress: 0.08, desc: "人員2：同上。AI駆動の改善（エージェント整備・spec運用・レビュープロセス）に充てる枠。／9/8-9/9分：確認事項ボードの整備（アーティファクトのボード化・ELI5タブ・コメント回収）1.53h（band AI稼働）。この枠の初回使用。" },
  ],
};
