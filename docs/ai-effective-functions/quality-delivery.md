...
| カテゴリ | チェックポイント | 内容・基準 |
| :--- | :--- | :--- |
| **仕様整合性** | **SSOT/User Storyとの一致** | コードが、ソースオブトゥルース（基準的な仕様）、仕様・要件定義ファイルの内容を正確に反映しているか。 |
...
> 従来型のアジャイルやスクラムといったプロジェクト形態の場合、1イテレーション( **2weeks** )単位でフィーチャーをデプロイし、エピックが整った時に Delivery が可能です。<br />
> spec駆動開発の場合、AIが担当できるフィーチャーは **2-3days** での実装を心掛けます。

spec駆動開発における「納期」は、単なる終了日ではなく、 **「開発サイクル（イテレーション）の速度」** として捉えます。<br />
下記の状態が生じている場合、AIに与えている **spec** の精度が悪いか、 **スコープ** が広すぎる可能性が高いと考えられます。
...
これらについて早期発見と改善を行うことで、開発後半に急激にデリバリーが劣化することを防止します。
