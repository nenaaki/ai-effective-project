...
## セッションの学習記録（LEARNING.md）

- `LEARNING.md`（リポジトリ直下）に、セッションごとの「使用者がイライラした部分」と「学びを得た部分」を記録する。
- **セッション開始時**: 事に当たる前に `LEARNING.md` を**全文読む**。読んでから作業方針を決める。過去に指摘された同じ失敗を繰り返さないためであり、省略しない。
- **セッション終了時**: そのセッションで発生した次の2点を追記する。何も無ければ追記しない。
  - **イライラした部分** — 指示の取り違え・余計な作業・確認不足・冗長な出力など、使用者が不満を示した箇所。何をされて不満だったかを具体的に書く。
  - **学びを得た部分** — 使用者の是正で判明した好み・判断基準・このリポジトリ固有の作法。次回そのまま適用できる形で書く。
- 書式は日付見出し（`## YYYY-MM-DD`）＋箇条書き。1件1行、原因と次回の振る舞いをセットで書く。事実だけを残し、言い訳や経緯の長文は書かない。
- **CLAUDE.md への昇格**: 同種の指摘・学びが**複数セッションで繰り返し（目安3回）**現れたら、単発の記録ではなく**恒久ルール**として `CLAUDE.md` の該当セクションに追記する。**昇格させた項目は `LEARNING.md` から削除する**（同じ内容を二重に持たない。`CLAUDE.md` が唯一の正本）。
